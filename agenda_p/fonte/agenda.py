...
class Agenda:
    ...
    def getQuantidadeDeFones(self, identificador: Identificador) -> int:
        if identificador is None:
            ...
            return sum(len([fone for fone in contato.getFones() if fone.identificador == identificador]) for contato in
                       self.contatos)
    ...
    def pesquisar(self, expressao:str) -> list:
        resultado = []
        for contato in self.contatos:
            if (expressao in contato.nome or any(expressao in fone.numero for fone in contato.getFones()) or any(
                    expressao in fone.identificador.value for fone in contato.getFones())):
